...
export default function InvoicesPage() {
  ...
  const handleAddInvoice = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (res.success) { 
      ...
      alert("Fatura resmi olarak mühürlendi ve GİB kuyruğuna iletildi! ??") 
    } else {
      alert(res.error)
    }
  }
  ...
  if (loading) return <div className="p-8 font-bold text-slate-500 animate-pulse">e-Dönüşüm Hub Yükleniyor...</div>

  return (
    <div className="flex flex-col xl:flex-row gap-6">
      
      {/* SOL KISIM: RESMİ FATURA KLASÖRLERİ VE TABLO */}
      <div className="flex-1 space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div className="bg-slate-900 text-white p-6 rounded-2xl shadow-sm border border-slate-800">
            <h3 className="text-slate-400 font-bold text-xs tracking-widest uppercase mb-1">Kesişen Giden Faturalar</h3>
          ...
          <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200">
            <h3 className="text-slate-500 font-bold text-xs tracking-widest uppercase mb-1">Alınan Gelen Faturalar</h3>
        ...
        <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
          <div className="bg-slate-50 px-6 py-4 border-b border-slate-200">
            <h3 className="font-bold text-slate-800">Resmi e-Fatura Evrak Akışı</h3>
          </div>
            ...
            <table className="w-full text-left text-sm">
              ...
              <tbody>
                {filteredInvoices.length === 0 ? (
                  <tr><td colSpan={5} className="p-12 text-center text-slate-400 font-bold">Bu klasörde henüz resmi fatura evrakı bulunmuyor.</td></tr>
                ) : (
                    ...
                    <tr key={invoice.id} className="border-b border-slate-100 hover:bg-slate-50 transition-colors">
                      ...
                      <td className="p-4">
                        {invoice.direction === 'OUTGOING' ? (
                          <span className="bg-emerald-100 text-emerald-700 px-2 py-0.5 rounded text-[9px] font-black uppercase tracking-wider">Giden (Satış)</span>
                        ) : (
                          <span className="bg-red-100 text-red-700 px-2 py-0.5 rounded text-[9px] font-black uppercase tracking-wider">Gelen (Alım)</span>
                        )}
                      </td>
                      <td className="p-4">
      ...
      {/* SAĞ KISIM: FATURA KESME SİHİRBAZI */}
        ...
        <div className="p-6">
          {activeTab === 'NEW_INVOICE' && (
            <form onSubmit={handleAddInvoice} className="space-y-4">
              <h3 className="font-bold text-slate-800 mb-2 flex items-center gap-2">?? e-Fatura Portalı</h3>
              ...
              <div>
                ...
                <select name="direction" required className="w-full px-3 py-2 border-2 border-slate-200 rounded-lg font-black text-sm text-slate-900 outline-none bg-slate-50">
                  <option value="OUTGOING">?? GİDEN FATURA (Satış / Bayiye)</option>
                  <option value="INCOMING">?? GELEN FATURA (Alım / Tedarikçiden)</option>
                </select>
              </div>
              ...
              <div>
                ...
                <select name="scenario" required className="w-full px-3 py-2 border-2 border-slate-200 rounded-lg font-bold text-sm text-slate-900 outline-none bg-slate-50">
                  <option value="COMMERCIAL">TİCARİ FATURA (Onay/Red Mekanizmalı)</option>
                  <option value="BASIC">TEMEL FATURA</option>
                  <option value="EARSIV">E-ARŞİV FATURA</option>
                </select>
              </div>

              <div>
                <label className="block text-xs font-bold text-slate-500 mb-1">İlgili Cari Firma</label>
              ...
              {/* DİNAMİK KDV SEÇİMLİ MATRAH ALANI */}
              <div className="flex gap-2">
                ...
                <div className="w-1/2">
                  <label className="block text-xs font-bold text-slate-500 mb-1">KDV Oranı</label>
                  <select name="taxRate" required className="w-full px-3 py-2 border-2 border-slate-200 rounded-lg font-black text-sm text-slate-900 outline-none bg-slate-50 focus:border-blue-500">
                    ...
                    <option value="10">%10 (Gıda / Tekstil / İlaç)</option>
                    <option value="1">%1 (Temel Tüketim / Unlu)</option>
                    <option value="0">%0 (Muafiyetli)</option>
                  </select>
                </div>
              </div>
              <p className="text-[10px] font-bold text-slate-400 mt-1">* Seçilen KDV oranına göre beyanname hacmi ve genel toplam otomatik hesaplanacaktır.</p>
          ...
          {activeTab === 'GIB_INFO' && (
            <div className="space-y-3 text-slate-600 font-medium text-xs leading-relaxed">
              ...
              <p>
                Sistemimiz, GİB'in yayınladığı en güncel UBL-TR 2.1 standardıyla tam uyumludur.
              </p>
              ...
              <p>
                Oluşturulan her fatura için seçtiğiniz vergi kırılımı işlenir ve benzersiz bir evrensel kod (UUID) mühürlenir. Bu sayece müşterinize sunum yaparken mali entegrasyon esnekliğimizi net bir şekilde ispat edebiliriz.
              </p>
            </div>
          )}
        </div>
