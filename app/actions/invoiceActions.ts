...
// 1. TÜM FATURALARI GETİR (Değişmedi, aynı kalıyor)
...
// 2. YENİ E-FATURA OLUŞTUR (DİNAMİK KDV SEÇİMLİ)
export async function createInvoice(formData: FormData) {
  try {
    ...
    // Kullanıcının arayüzden seçtiği KDV oranını alıyoruz (Örn: 20, 10, 1)
    const taxRate = parseFloat(formData.get('taxRate') as string) || 20 

    if (totalAmount <= 0) return { success: false, error: "Fatura tutarı 0'dan büyük olmalıdır." }
    if (!companyId) return { success: false, error: "Lütfen ilgili cari hesabı seçin." }
    ...
    return { success: false, error: "Fatura resmi sisteme kaydedilirken bir hata oluştu." }
  }
}